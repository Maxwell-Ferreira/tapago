...
export default class ExpensesController {
  ...
  private getExpenses(type: string, user?: User) {
    ...
    return new QueryFilter(query, {}).execute();
  }
  ...
  public async headerInfo({ auth, request }: HttpContextContract) {
    const competenceValidation = await request
      .validate({
    ...
    if (!competenceValidation.competence) {
      competence = `${now.getFullYear()}-${(now.getMonth() + 1)
        .toString()
        .padStart(2, '0')}`;
    } else {
      competence = format(
        competenceValidation.competence.toJSDate(),
        'yyyy-MM',
      );
    }
    ...
    const totalSingle = await Database.from('expenses')
      .sum('amount as total')
      .where({ user_id: auth.user?.id, type: 'SINGLE' })
    ...
    const totalFixedsQuery = Database.from('expenses')
      .sum('amount as total')
      .where({ user_id: auth.user?.id, type: 'FIXED' });
    ...
    const totalFixeds = await totalFixedsQuery
      .exec()
      .then((resp) => Number(resp[0]?.total) || 0);

    const monthlyLimit = Number(auth.user?.monthlyLimit || 0);
